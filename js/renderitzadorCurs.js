document.addEventListener('DOMContentLoaded', () => {
    ...
    // NUEVO HELPER: Limpieza de texto para Flashcards
    ...
    async function cargarDatos() {
        ...
        // Pequeña protección: si moduls es null, lo ponemos como array vacío para que no explote
    ...
    async function guardarProgreso(progresoObj) {
        ...
        // Marcamos completado si llega al 100%
    ...
    function estaBloqueado(indexModulo) {
        ...
        // Protección si progreso no está sincronizado
        if (!state.progreso.modulos) return true;
    ...
    function renderSidebar() {
        ...
        // 2. MÓDULOS - CORRECCIÓN CRÍTICA: Añadido chequeo IF para evitar "forEach of undefined"
        if (state.curso.modulos && state.curso.modulos.length > 0) {
    ...
    // CORRECCIÓN 2: Arreglado el renderSubLink para que compare Strings y marque active
    ...
    function renderMainContent() {
        ...
        if (state.currentView === 'examen_final') {
            ...
            renderSidebarTools(gridRight, { titol: 'Examen Final' });
            return;
        }

        // VISTAS MÓDULO NORMAL
        const mod = state.curso.modulos ? state.curso.modulos[state.currentModuleIndex] : null;

        if (!mod) {
            container.innerHTML = "<p>Contingut no trobat.</p>";
            return;
        }
        ...
        if (state.currentView === 'teoria') {
            ...
            // AQUÍ LLAMAMOS A LA NUEVA FUNCIÓN GAMIFICADA
    ...
    // ------------------------------------------------------------------------
    // CORRECCIÓN 1: FLASHCARDS GAMIFICADAS
    // ------------------------------------------------------------------------
    function renderFlashcards(container, cards) {
        ...
        cards.forEach((card, idx) => {
            // LIMPIEZA DE TEXTO (Usamos la nueva función helper)
            ...
            let questionHtml = words.map((w, i) => {
                 if(i === hiddenIndex) return `<span class="cloze-blank">_______</span>`;
                 return w;
            }).join(" ");
            ...
            let btnsHtml = options.map(opt => {
                ...
                let targetSafe = wordClean.replace(/'/g, "\\'");
                return `<button class="btn-flash-option" onclick="checkFlashcard(this, '${optSafe}', '${targetSafe}')">${opt}</button>`;
            }).join('');

            let backContent = '';
            if (state.godMode) {
                backContent = `<div style="padding:15px; color:white; font-size:0.95rem;">${cleanText}</div>`;
            } else {
                backContent = `
    ...
    // FUNCION GLOBAL CHECK FLASHCARD
    window.checkFlashcard = function(btn, selected, correct) {
        ...
        if (selected === correct) {
            ...
            // Auto-corregir visualmente tras error
            setTimeout(() => {
                blankSpan.innerText = correct;
                blankSpan.style.color = '#fff'; 
