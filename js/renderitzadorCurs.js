document.addEventListener('DOMContentLoaded', () => {
    ...
    async function init() {
        ...
        if (container) container.innerHTML = '<div class="loader"></div><p class="loading-text">Sincronitzant contingut...</p>';
        try {
            await cargarDatos();
            ...
            console.error(e);
            if (container) container.innerHTML = `<div class="dashboard-card" style="border-top:4px solid red; padding:20px;"><h3>Error de Connexió</h3><p>${e.message}</p><button class="btn-secondary" onclick="window.location.reload()">Reintentar</button></div>`;
        }
    }

    // --- CAMBIO CLAVE: DOBLE PETICIÓN PARA GARANTIZAR DATOS ---
    async function cargarDatos() {
        // 1. Obtener la MATRÍCULA (Solo nos interesa ID y Progreso)
        const queryMatricula = `filters[users_permissions_user][id][$eq]=${USER.id}&filters[curs][slug][$eq]=${SLUG}`;
        const resMat = await fetch(`${STRAPI_URL}/api/matriculas?${queryMatricula}`, {
            headers: { 'Authorization': `Bearer ${TOKEN}` }
        });
        const jsonMat = await resMat.json();
        
        if (!jsonMat.data || jsonMat.data.length === 0) throw new Error("No estàs matriculat en aquest curs.");
        const matriculaData = jsonMat.data[0];

        state.matriculaId = matriculaData.documentId || matriculaData.id;
        state.progreso = matriculaData.progres_detallat || {};

        // 2. Obtener el CURSO DIRECTAMENTE (Para garantizar que bajen los módulos)
        // Pedimos directamente a la colección 'cursos', así Strapi no corta la relación.
        const queryCurso = [
            `filters[slug][$eq]=${SLUG}`,
            `populate[moduls][populate][preguntes][populate][opcions]=true`,
            `populate[moduls][populate][material_pdf]=true`,
            `populate[moduls][populate][targetes_memoria]=true`,
            `populate[examen_final][populate][opcions]=true`,
            `populate[imatge]=true`
        ].join('&');

        const resCur = await fetch(`${STRAPI_URL}/api/cursos?${queryCurso}`, {
            headers: { 'Authorization': `Bearer ${TOKEN}` }
        });
        const jsonCur = await resCur.json();

        if (!jsonCur.data || jsonCur.data.length === 0) throw new Error("Error carregant el contingut del curs.");
        
        // Guardamos los datos del curso limpio
        state.curso = jsonCur.data[0];

        // 3. Normalización de Módulos (Strapi v5 Array fix)
        let rawMods = state.curso.modulos;
        if (!rawMods) {
            state.curso.modulos = [];
        } else if (rawMods.data) {
            state.curso.modulos = rawMods.data; // Si viene dentro de .data
        } else if (!Array.isArray(rawMods)) {
            state.curso.modulos = [];
        }
        
        // Ahora state.curso.modulos tiene los datos reales de tu captura de pantalla
    }

    async function inicializarProgresoEnStrapi() {
        const mods = state.curso.modulos; 
        const nuevoProgreso = {
            modulos: mods.map(() => ({ aprobado: false, nota: 0, intentos: 0 })),
    ...
    async function guardarProgreso(progresoObj) {
        ...
        let porcentaje = 0;
        if (totalModulos > 0) {
            porcentaje = Math.round((aprobados / totalModulos) * 100);
        } else {
            porcentaje = (progresoObj.examen_final && progresoObj.examen_final.aprobado) ? 100 : 0;
        }
    ...
    function renderSidebar() {
        ...
        if (mods.length > 0) {
            mods.forEach((mod, idx) => {
             ...
             html += `<div style="padding:15px; text-align:center; color:#666; font-size:0.9rem;">(Sense mòduls carregats)</div>`;
        }
        ...
        let allPassed = true;
        if (mods.length > 0) {
            allPassed = state.progreso.modulos && state.progreso.modulos.every(m => m.aprobado);
        }
        const finalLocked = !state.godMode && !allPassed;
    ...
    function renderMainContent() {
        ...
        if (state.currentView === 'intro') {
            container.innerHTML = `
                ...
                <div class="module-content-text" style="margin-top:20px;">
                    ${parseStrapiRichText(state.curso.descripcio || "Benvingut al curs.")}
                </div>`;
            renderSidebarTools(gridRight, { titol: 'Inici' }, false);
            return;
        }
        ...
        if (state.currentView === 'glossary') {
            ...
            renderSidebarTools(gridRight, { titol: 'Glossari' }, false);
            return;
        }
        ...
        if (state.currentView === 'examen_final') {
            ...
            renderSidebarTools(gridRight, { titol: 'Examen Final' }, true);
            return;
        }

        // 4. MÓDULO
        const mods = state.curso.modulos;
        const mod = mods[state.currentModuleIndex];

        if (!mod) {
            container.innerHTML = `<div class="alert-info">Selecciona un mòdul del menú lateral.</div>`;
            return;
        }

        if (state.currentView === 'teoria') {
            renderTeoria(container, mod);
            renderSidebarTools(gridRight, mod, false);
            ...
            const pMod = (state.progreso.modulos && state.progreso.modulos[state.currentModuleIndex]) 
                         ? state.progreso.modulos[state.currentModuleIndex] 
                         : { aprobado: false };
            
            const aprobado = pMod.aprobado;
    ...
    function renderSidebarTools(container, mod, hideTools) {
    ...
    function renderTestIntro(container, mod, modIdx) {
        const prog = (state.progreso.modulos && state.progreso.modulos[modIdx]) 
                      ...
                      : { aprobado: false, intentos: 0, nota: 0 };
    ...
    window.entregarTest = function() {
        window.mostrarModalConfirmacion("Entregar", "Segur que vols finalitzar?", async () => {
            ...
            if (!state.progreso.modulos[modIdx]) {
                state.progreso.modulos[modIdx] = { intentos:0, nota:0, aprobado:false };
            }
